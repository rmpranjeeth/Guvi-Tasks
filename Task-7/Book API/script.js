const URLbooks = "https://www.googleapis.com/books/v1/volumes?q=__title__+inauthor:__author__&startIndex=__start__&maxResults=15&langResrict=en&key=__apiKey__";
const URLauth = "https://www.googleapis.com/books/v1/volumes?q=+inauthor:__author__&startIndex=__start__&maxResults=15&langResrict=en&key=__apiKey__";
...
function loadData(title, author, start) {
    fetch(URLbooks.replace("__title__", title)
    ...
    .then(data => {
        ...
        for (let i = 0; i <= authorsHTML.length - 1; i++) {
            authorsHTML[i].addEventListener("click", (e) => {
                ...
                fetch(URLbooks.replace("__author__", e.target.innerText)
                    .replace("__title__", "")
                    .replace("__start__", 0)
                    .replace("__apiKey__", apiKey))
                    .then(response => response.json())
                    .then(data => {
                        ...
                        mainContainer.insertBefore(authorPgHeading, mainContainer.children[1]);
                        mainContainer.insertBefore(hrLineTop, mainContainer.children[1]);
                        resultsHTML.innerHTML = ""
                        authorPgHeading.innerHTML = "Listing by author:" + e.target.innerText
                        displayResults(data.items)
                    })
            })
        } 
